...
class Link():
    ...
    def __init__(self, occurrence: adsk.fusion.Occurrence) -> None:
        ...
        self.mesh_format = "stl"  # default to STL, can be "obj" or "stl"
    ...
    def get_baked_mesh_body(self, body: adsk.fusion.BRepBody, bake_transform: adsk.core.Matrix3D) -> adsk.fusion.BRepBody:
        """
        Create a temporary copy of a body and apply a transform to it.
        This allows exporting the mesh pre-positioned to its joint frame.
        ...
        adsk.fusion.BRepBody
            The temporary transformed body
        """
        try:
            from . import mesh_baker
            return mesh_baker.get_baked_mesh_body(body, bake_transform)
        except Exception as e:
            return body

    def calculate_bake_transform(self) -> adsk.core.Matrix3D:
        """
        Calculate the transform to bake this link's mesh to its joint frame.
        ...
        try:
            from . import mesh_baker
            parent_joint = self.get_parent_joint()
            return mesh_baker.calculate_bake_transform(self.link, parent_joint)
        except Exception as e:
            return adsk.core.Matrix3D.create()

    def get_transformed_com(self, bake_transform: adsk.core.Matrix3D) -> list:
        """
        Calculate the Center of Mass position after applying the bake transform.
        Used for updating inertial origin in URDF/SDF/MJCF.
        
        Parameters:
        -----------
        bake_transform: adsk.core.Matrix3D
            The transform matrix applied to the link
        
        Returns:
        --------
        list: [x, y, z] in meters, expressed in the new frame
        """
        try:
            from . import mesh_baker
            original_com = self.phyPro.centerOfMass
            return mesh_baker.transform_point(original_com, bake_transform)
        except Exception as e:
            # Return original CoM if transformation fails
            return [
                self.phyPro.centerOfMass.x * 0.01,
                self.phyPro.centerOfMass.y * 0.01,
                self.phyPro.centerOfMass.z * 0.01
            ]
