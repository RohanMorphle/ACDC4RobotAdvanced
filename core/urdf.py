...
class URDF():
    ...
    def get_link_element(self, link: Link) -> Element:
        ...
        scale_str = "0.01 0.01 0.01" if getattr(link, 'mesh_format', 'stl') == 'obj' else "0.001 0.001 0.001"
        mesh_v.attrib = {"filename": self.get_link_visual_geo(link), "scale": scale_str}
        ...
        scale_str = "0.01 0.01 0.01" if getattr(link, 'mesh_format', 'stl') == 'obj' else "0.001 0.001 0.001"
        mesh_c.attrib = {"filename": self.get_link_collision_geo(link), "scale": scale_str}
    ...
    def get_link_collision_geo(self, link: Link) -> str:
        ...
        if (visual_body is None) and (col_body is None):
            ...
            mesh_loc = "meshes/" + link.get_name() + "_collision" + ext
            return mesh_loc
        elif (visual_body is None) and (col_body is not None):
            error_message = "Please set two bodies, one for visual and one for collision. \n"
            error_message = error_message + link.get_name() + " body for visual missing."
...
def get_link_element(link: Link) -> Element:
    ...
    mesh_v = SubElement(geometry_v, "mesh")
    scale_str = "0.01 0.01 0.01" if getattr(link, 'mesh_format', 'stl') == 'obj' else "0.001 0.001 0.001"
    mesh_v.attrib = {"filename": get_link_visual_geo(link), "scale": scale_str}
    ...
    mesh_c = SubElement(geometry_c, "mesh")
    scale_str = "0.01 0.01 0.01" if getattr(link, 'mesh_format', 'stl') == 'obj' else "0.001 0.001 0.001"
    mesh_c.attrib = {"filename": get_link_collision_geo(link), "scale": scale_str}
