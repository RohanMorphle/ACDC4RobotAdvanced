...
def get_link_visual_geo(link: Link) -> str:
    ...
    if (visual_body is None) and (col_body is None):
        ...
        mesh_loc = "model://" + robot_name + "/meshes/" + link.get_name() + ".obj"
        return mesh_loc
    elif (visual_body is not None) and (col_body is not None):
        mesh_loc = "model://" + robot_name + "/meshes/" + link.get_name() + "_visual.obj"
        return mesh_loc
...
def get_link_collision_geo(link: Link) -> str:
    ...
    if (visual_body is None) and (col_body is None):
        ...
        mesh_loc = "model://" + robot_name + "/meshes/" + link.get_name() + "_collision.obj"
        return mesh_loc
    elif (visual_body is None) and (col_body is not None):
        error_message = "Please set two bodies, one for visual and one for collision. \n"
        error_message = error_message + link.get_name() + " body for visual missing."
...
def get_link_element(link: Link) -> Element:
    ...
    # STL: 0.01, OBJ: 0.1 (10x larger)
    visual_scale = "0.1 0.1 0.1" if link.mesh_format == "obj" else "0.01 0.01 0.01"
    vis_geo_mesh_scale.text = visual_scale
    ...
    # STL: 0.01, OBJ: 0.1 (10x larger)
    collision_scale = "0.1 0.1 0.1" if link.mesh_format == "obj" else "0.01 0.01 0.01"
    col_geo_mesh_scale.text = collision_scale
